...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
 switch (uMsg){
        ...
        case WM_CREATE:
        {
            ...
            //NOTE: if you set bitCount <= 8, then you must use a color table, and provide
            //bmi info with the color table. so we don't duplicate colors
            //we won't do that - if we need to reduce executable size - we should though. 
        ...
        //destroy is for the window's cleanup - child windows etc, ncdestroy is for the window itself
        case WM_DESTROY: {
            PostQuitMessage(0);
            return 0;
        }
